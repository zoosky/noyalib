...
//!
//! A [`Policy`] inspects parser events as the document is loaded
//! and rejects any that violate organisational constraints — common
//! examples are "no anchors", "no custom tags", or "no scalar
//! larger than N bytes". Policies fire on the AST loader path; if
//! any policy is registered the streaming fast-path is bypassed
//! automatically so the policy contract is honoured everywhere.
//!
//! Built-in policies live in this module:
//!
//! - [`DenyAnchors`] — reject any document that defines or
//!   dereferences an anchor.
//! - [`DenyTags`] — reject any tagged scalar / collection.
//! - [`MaxScalarLength`] — cap individual scalar length in bytes.
//!
