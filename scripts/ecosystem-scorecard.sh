...
#   scripts/ecosystem-scorecard.sh --repo noyalib  # one repo (repeatable)
#   scripts/ecosystem-scorecard.sh --json out.json
#   scripts/ecosystem-scorecard.sh --inject extra.json
#   scripts/ecosystem-scorecard.sh --floor 0.95    # exit 1 below this score
#   scripts/ecosystem-scorecard.sh --timeout 300 --build-timeout 900
#
...
probe_repo() {
  ...
  local brc=$?
  PROBE_TIMEOUT="$saved"
  # The recorded probe string is the command that actually ran, spaces
  # squeezed by `echo`'s word splitting. It used to substitute a literal
  # `--default-features` when no feature args were in play — a flag cargo
  # does not accept, so the one thing ECOSYSTEM.md promises ("the command
  # is printed next to the number") could not be copied and run.
  if [ "$brc" -ne 0 ]; then
    local why; why=$([ "$brc" = "124" ] && echo "build timeout after ${BUILD_TIMEOUT}s" || echo "build failed (rc=$brc)")
    record "$repo" tests correctness 5 "$why" 0 "$(echo cargo test --workspace --locked $fa --no-run)"
    return_from_tests=1
  fi
  ...
  local probe; probe="$(echo cargo test --workspace --locked $fa)"
  ...
  record "$repo" clippy code_health 3 "$cwarn diagnostics (rc=$crc)" \
    "$(bool_score "$crc")" "$(echo cargo clippy --workspace --all-targets --locked $fa -- -D warnings)"
...
if [ -n "$INJECT" ]; then
  if [ ! -f "$INJECT" ]; then
    ...
    BLOCK="$WORK/block.md"
    {
      ...
      # A probe command is a pipeline as often as not, and a bare `|`
      # inside a cell ends that cell: the renderer drops everything
      # after it and markdownlint reports MD056.
      #
      # Escape via split/join, not gsub — a gsub replacement string is
      # itself scanned for `\` and `&`, and the number of backslashes
      # that survive differs between implementations (BSD awk emitted
      # `\\|` where the intent was `\|`). Joining with a literal keeps
      # the escape as data, so the row is identical under every awk.
      awk -F'\t' '
        function esc(s,   n, parts, out, i) {
          n = split(s, parts, "|")
          out = parts[1]
